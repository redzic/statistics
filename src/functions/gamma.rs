...
pub fn gamma(mut x: f64) -> f64 {
    ...
    if ix >= 0x40670000 {
        ...
        if sign {
            ...
            unsafe {
                std::ptr::read_volatile(&((x1p_126 / x) as f32));
            }
    ...
    if x < 0.0 {
        ...
        r = -std::f64::consts::PI
            / ((absx * std::f64::consts::PI).sin() * absx * r);
        dy = -dy;
        z = -z;
    }
    r += dy * (GMHALF + 0.5) * r / y;
...
pub fn lngamma(mut x: f64) -> (f64, i32) {
    /* lgamma_r(x, signgamp)
    ...
    let mut signgam: i32 = 1;
    let sign = (u >> 63) != 0;
    let ix = ((u >> 32) as u32) & 0x7fffffff;
    if ix >= 0x7ff00000 {
        return (x * x, signgam);
    }
    if ix < (0x3ff - 70) << 20 {
